...
function fetchPortraits() {
  fetch('characters.json')
    ...
    .then(data => {
      characters = Object.values(data).filter(character => character.role === 'survivor');
      startRandomizer();  // Start randomizer after fetching portraits
    })
    .catch(error => {
...
function startRandomizer() {
  if (characters.length === 0) {
    console.error('No survivor portraits available');
    return;
  }
